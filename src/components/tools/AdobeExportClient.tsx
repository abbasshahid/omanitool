...
interface AdobeExportClientProps {
  toolId: 'pdf-to-word' | 'pdf-to-powerpoint' | 'pdf-to-excel';
}

export default function AdobeExportClient({ toolId }: AdobeExportClientProps) {
  ...
  const handleProcess = async () => {
    ...
    try {
      const targetFormat = toolId === 'pdf-to-word' ? 'DOCX' : toolId === 'pdf-to-powerpoint' ? 'PPTX' : 'XLSX';
      const fileExt = toolId === 'pdf-to-word' ? '.docx' : toolId === 'pdf-to-powerpoint' ? '.pptx' : '.xlsx';
      ...
      if (!response.ok) {
        throw new Error(await response.ok ? '' : await response.text());
      }
  ...
  const downloadExt = toolId === 'pdf-to-word' ? '.docx' : toolId === 'pdf-to-powerpoint' ? '.pptx' : '.xlsx';
              ...
              <div className="space-y-6">
                ...
                {!isDone ? (
                  <div className="pt-4 border-t border-[var(--color-border-base)] flex justify-end">
                    ...
                    >
                      {isProcessing ? (
                        <span className="flex items-center justify-center gap-2"><RefreshCw className="w-4 h-4 animate-spin" /> Converting via Adobe API...</span>
                      ) : (
                        tool.name + " Now"
                      )}
                    </button>
                  ...
                  <div className="pt-6 border-t border-[var(--color-border-base)] flex flex-col items-center justify-center p-8 bg-emerald-500/5 border border-emerald-500/20 rounded-xl text-center">
                    ...
                    <p className="text-[var(--color-text-muted)] mb-6 max-w-sm">Your PDF has been successfully converted and is ready for download.</p>
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 w-full justify-center">
                       ...
                       {downloadUrl && (
                         <a href={downloadUrl} download={file.name.replace('.pdf', downloadExt)} className="btn-primary bg-emerald-600 hover:bg-emerald-500 ring-emerald-600 shadow-lg shadow-emerald-500/20 px-8 py-3 rounded-xl gap-2 font-bold text-white flex items-center justify-center">
                            <Download className="w-5 h-5" /> Download {downloadExt}
                         </a>
                       )}
                    </div>
                  </div>
