...
import { RefreshCw, Download, FileText, Delete } from 'lucide-react';
...
export default function RemovePagesClient() {
      ...
      if (part.includes('-')) {
        ...
        if (!isNaN(start) && !isNaN(end)) {
          for (let i = start; i <= end; i++) {
             if (i > 0 && i <= totalCount) pagesToRemove.add(i - 1);
          }
        }
      } else {
        const num = parseInt(part);
        if (!isNaN(num) && num > 0 && num <= totalCount) {
          pagesToRemove.add(num - 1);
        }
      }
    }
  ...
  const handleProcess = async () => {
    ...
    if (!removeString.trim()) {
      alert("Please enter the page numbers you want to remove.");
      return;
    }
    ...
    try {
      ...
      const pagesToRemove = parsePageNumbers(removeString, totalPages);
      ...
      console.error(e);
      alert('Failed to remove pages. The PDF may be encrypted.');
    } finally {
      setIsProcessing(false);
    }
  };
  ...
  return (
    <div className="container mx-auto max-w-7xl px-4 xl:px-8 py-10 md:py-16">
      ...
      <div className="mb-10 text-center">
        <AdSenseBanner dataAdSlot="REMOVE_TOP" className="h-[90px] w-full max-w-[728px] mx-auto bg-slate-900/30 rounded-xl" />
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-10">
                ...
                {!resultUrl ? (
                  <div className="pt-4 border-t border-[var(--color-border-base)] flex flex-col items-end gap-4">
                    ...
                    <div className="w-full">
                       <label className="block text-sm font-medium text-[var(--color-text-main)] mb-2">Pages to Remove</label>
                       <input 
                         type="text" 
                         value={removeString}
                         onChange={(e) => setRemoveString(e.target.value)}
                         placeholder="e.g. 1, 3, 5-8"
                         className="w-full px-4 py-3 bg-[var(--color-background-base)] border border-[var(--color-border-base)] rounded-xl text-[var(--color-text-main)] focus:ring-2 focus:ring-red-500 focus:outline-none transition-shadow"
                       />
                    ...
                    >
                      {isProcessing ? (
                        <span className="flex items-center justify-center gap-2"><RefreshCw className="w-4 h-4 animate-spin" /> Removing...</span>
                      ) : (
                        "Remove Pages Now"
                      )}
                    </button>
                  ...
                  <div className="pt-6 border-t border-[var(--color-border-base)] flex flex-col items-center justify-center p-8 bg-emerald-500/5 border border-emerald-500/20 rounded-xl text-center">
                    ...
                    <h3 className="text-xl font-bold text-[var(--color-text-main)] mb-2">Pages Removed!</h3>
                    <p className="text-[var(--color-text-muted)] mb-6 max-w-sm">The selected pages were securely removed from your document.</p>
                    
                    <div className="flex flex-col sm:flex-row gap-4 w-full justify-center">
                       <button onClick={resetTool} className="text-sm text-gray-500 font-medium hover:text-[var(--color-text-main)]">
                          Start Over
                       </button>
                       <a href={resultUrl} download={`cleaned-${file.name}`} className="btn-primary bg-emerald-600 hover:bg-emerald-500 ring-emerald-600 shadow-lg shadow-emerald-500/20 px-8 py-3 rounded-xl gap-2 font-bold text-white flex items-center justify-center">
        ...
        <aside className="w-full lg:w-[336px] flex-shrink-0 space-y-8">
           <AdSenseBanner dataAdSlot="REMOVE_SIDEBAR" className="h-[280px] w-full bg-slate-900/50 rounded-xl border border-[var(--color-border-base)]" />
        </aside>
