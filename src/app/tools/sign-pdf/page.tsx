...
import { RefreshCw, Download, FileText, PenTool } from 'lucide-react';
...
export default function GenericToolPage() {
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      // Stamp on the last page for the prototype
      ...
      console.error(e);
      alert('Failed to sign PDF. Ensure the file is valid and not encrypted.');
    } finally {
          ...
          <div className="card-container p-6 md:p-8">
            {!file ? (
              ...
              <div className="space-y-6">
                ...
                {!resultUrl && (
                   <div className="bg-blue-50 dark:bg-blue-500/5 border border-blue-100 dark:border-blue-800/30 rounded-xl p-6">
                      <div className="flex items-start gap-4">
                         <div className="p-2 bg-blue-100 dark:bg-blue-900/50 rounded-lg text-blue-600 dark:text-blue-400 mt-1">
                            <PenTool className="w-5 h-5" />
                         </div>
                         <div className="flex-1 space-y-4">
                            <div>
                               <h3 className="font-semibold text-[var(--color-text-main)] mb-1">Draw Signature</h3>
                               <p className="text-sm text-[var(--color-text-muted)]">Draw your signature below. It will be added to the bottom of the last page.</p>
                            </div>
                            
                            <div className="space-y-4">
                               <div className="border-2 border-dashed border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 rounded-xl overflow-hidden shadow-inner">
                                  <SignatureCanvas 
                                     ref={sigCanvas}
                                     penColor="black"
                                     canvasProps={{className: 'w-full h-48 sm:h-64 signature-canvas'}} 
                                  />
                               </div>
                               <div className="flex justify-end">
                                  <button onClick={() => sigCanvas.current?.clear()} className="text-sm text-gray-500 hover:text-[var(--color-text-main)] pb-2 flex items-center gap-1">
                ...
                {!resultUrl ? (
                  <div className="pt-6 border-t border-[var(--color-border-base)] flex justify-end">
                    ...
                    >
                      {isProcessing ? (
                        <span className="flex items-center justify-center gap-2"><RefreshCw className="w-4 h-4 animate-spin" /> Signing...</span>
                      ) : (
                        "Sign Document"
                      )}
                    </button>
                  ...
                  <div className="pt-6 border-t border-[var(--color-border-base)] flex flex-col items-center justify-center p-8 bg-emerald-500/5 border border-emerald-500/20 rounded-xl text-center">
                    ...
                    <h3 className="text-xl font-bold text-[var(--color-text-main)] mb-2">Document Signed!</h3>
                    <p className="text-[var(--color-text-muted)] mb-6 max-w-sm">Your signature has been applied to the last page.</p>
                    
                    <div className="flex flex-col sm:flex-row gap-4 w-full justify-center">
                       <button onClick={resetTool} className="text-sm text-gray-500 font-medium hover:text-[var(--color-text-main)]">
                          Start Over
                       </button>
                       <a href={resultUrl} download={`signed-${file.name}`} className="btn-primary bg-emerald-600 hover:bg-emerald-500 ring-emerald-600 shadow-lg shadow-emerald-500/20 px-8 py-3 rounded-xl gap-2 font-bold text-white flex items-center justify-center">
